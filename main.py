...
def processFile(update,bot,message,file,thread=None,jdb=None):
    ...
    if file_size > max_file_size:
        ...
        zip.close()
        mult_file.close()
        client,cli = processUploadFiles(file,file_size,mult_file.files,update,bot,message,jdb=jdb)
        try:
            os.unlink(file)
        except:pass
        file_upload_count = len(zipfile.files)
    else:
        client,cli = processUploadFiles(file,file_size,[file],update,bot,message,jdb=jdb)
        file_upload_count = 1
    ...
    if client:
        ...
        if len(files)>0:
            urls = []
            for item in files:
                urls.append(item['directurl'])
            try:
                urls = cli.parse_profile(urls)
                for u in urls:
                    profiles.append(u)
            except Exception as ex:
                print(str(ex))
...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        tl_admin_user = os.environ.get('tl_admin_user','obisoftt')

        #set in debug
        tl_admin_user = os.environ.get('administrador','MarlonSu')
        ...
        if '/myuser' in msgText:
            message = bot.sendMessage(update.message.chat.id,'⚙️Extrayendo Info...')
            quotetext = ''
            try:
                proxy = ProxyCloud.parse(user_info['proxy'])
                host = user_info['moodle_host']
                ...
                loged = client.login()
                if loged:
                    quotetext = client.get_quote()
            except:pass
            getUser = user_info
            if getUser:
                statInfo = infos.createStat(username,getUser,jdb.is_admin(username),quote=quotetext)
                bot.editMessageText(message,statInfo)
                return
        if '/zips' in msgText:
            getUser = user_info
            if getUser:
        ...
        if '/start' in msgText:
            ...
            start_msg+= '🦾Desarrollador: ༺ @obisoftt ༻\n\n'
...
def main():
    bot_token = '5341485075:AAGMQSEmY0KXW6r0vvzJGQoSGQ2S4XEhecw'
