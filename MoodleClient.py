...
class MoodleClient(object):
    ...
    def parse_profile(self,urls):
        ...
        resp = self.session.get(profile,proxies=self.proxy)
        soup = BeautifulSoup(resp.text,'html.parser')
        ...
        resp = self.session.post(profile,data=payload,proxies=self.proxy)
        token = ''
        userdata = self.getUserData()
        if userdata:
            if 'token' in userdata:
...
#client = MoodleClient('sdel','Ana720209***','https://moodle.uclv.edu.cu/',repo_id=4)
#loged = client.login()
#if loged:
#    resp,data = client.upload_file_calendar('requirements.txt')
#    print(data)
